---
description: Remove an unstarted future phase from the project roadmap, delete its directory, renumber all subseq
---
<purpose>
...
```bash
INIT=$(node /Users/satori/.antigravity/get-shit-done/bin/gsd-tools.js init phase-op "${target}")
```
...
```bash
node /Users/satori/.antigravity/get-shit-done/bin/gsd-tools.js commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
